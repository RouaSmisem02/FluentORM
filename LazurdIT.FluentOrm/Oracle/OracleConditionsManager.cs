...
namespace LazurdIT.FluentOrm.Oracle
{
    public class OracleConditionsManager<T> : IFluentConditionsManager<T> where T : IFluentModel, new()
    {
        ...
        public virtual OracleConditionsManager<T> NotEq<TProperty>(Expression<Func<T, TProperty>> property, TProperty value)
        {
            WhereConditions.Add(OracleOp.NotEq(property, value));
            return this;
        }
