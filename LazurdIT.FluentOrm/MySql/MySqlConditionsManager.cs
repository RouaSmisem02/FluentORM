...
namespace LazurdIT.FluentOrm.MySql
{
    public class MySqlConditionsManager<T> : IFluentConditionsManager<T> where T : IFluentModel, new()
    {
        ...
        public virtual MySqlConditionsManager<T> NotEq<TProperty>(Expression<Func<T, TProperty>> property, TProperty value)
        {
            WhereConditions.Add(MySqlOp.NotEq(property, value));
            return this;
        }
